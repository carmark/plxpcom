...
/**
 * XXX - Original notices maintained because the only thing changed 
 * for this test is the contract ID. Matt Kennedy <matt@jumpline.com>
 */
...
#define NS_SAMPLE_CONTRACTID "@mozilla.org/perlsample;1"
...
main(void)
{
    ...
    nsCOMPtr<nsIComponentManagerObsolete> cReg = nsnull;
    nsCOMPtr<nsIComponentManager> cMgr = nsnull;
    rv = NS_GetComponentManager(getter_AddRefs(cMgr));
    cReg = do_QueryInterface(cMgr, &rv);
    cReg->AutoRegister(0, nsnull);
